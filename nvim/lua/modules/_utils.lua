...
local to_rgb = function(hex)
    ...
    if #hex == 9 then
        ...
        return string.format("rgba(%s, %s, %s, %s)", tonumber("0x" .. red),
                             tonumber("0x" .. green), tonumber("0x" .. blue),
                             tonumber("0x" .. alpha))
    end

    _, red, green, blue = hex:match("(.)(..)(..)(..)")
    return string.format("rgb(%s, %s, %s)", tonumber("0x" .. red),
                         tonumber("0x" .. green), tonumber("0x" .. blue))
end
...
Util.get_word = function()
    ...
    local current_word = fn.getline(first_line, last_line)[1]:sub(first_col,
                                                                  last_col)
...
Util.strike_through = function()
    ...
    vim.api.nvim_buf_add_highlight(0, strike_ns, "StrikeThrough",
                                   first_line - 1, first_col - 1, last_col)
end
...
-- For creating augroups
Util.nvim_create_augroups = function(definitions)
